...
## 9. Native izinler (konum, kamera vb.)

Web sitesindeki `navigator.geolocation`, `<input type="file" capture>` gibi tarayıcı API'leri, Capacitor'ın WebView'ı içinde çalışırken **native Android izin sistemine bağlanabilmesi için AndroidManifest.xml'de ilgili iznin tanımlı olmasını ister.** Tanımlı değilse: tarayıcı hiçbir izin penceresi göstermeden isteği sessizce reddeder (kullanıcı hiçbir şey görmez, sadece "konum alınamadı" gibi bir hata döner) — bu bir kod hatası değil, eksik manifest izni belirtisidir.

**Belirti:** Uygulamada konum/kamera özelliği web sürümünde çalışıyor ama APK'da tıklayınca hiç izin penceresi çıkmıyor, direkt başarısız oluyor.

**Çözüm:** `android/app/src/main/AndroidManifest.xml` içinde `<application` etiketinden ÖNCE (dosyanın üst kısmı) ilgili izinleri ekle:

```xml
<!-- Konum (navigator.geolocation kullanan her proje için) -->
<uses-permission android:name="android.permission.ACCESS_FINE_LOCATION" />
<uses-permission android:name="android.permission.ACCESS_COARSE_LOCATION" />

<!-- Kamera (fotoğraf çekme özelliği varsa) -->
<uses-permission android:name="android.permission.CAMERA" />
```

Ekledikten sonra APK'yı yeniden derlemek şart (manifest değişikliği çalışma anında değil, derleme anında etkili olur). Web sitesinin kendisinde hiçbir kod değişikliği gerekmez — `navigator.geolocation.getCurrentPosition()` aynen çalışmaya devam eder, Capacitor'ın WebView köprüsü izni manifest'ten okuyup native pencereyi otomatik açar.
...
## 10.5. Windows'ta "Unable to delete directory" kilit hatası (Patika 1.9.2'de acı çekilerek öğrenildi)

Android Studio veya CLI ile derlerken şu hata tekrar tekrar, **her seferinde farklı bir dosyada** çıkarsa:
```
Unable to delete directory '...\build\intermediates\...\out'
Failed to delete some children. This might happen because a process has files open...
```
Bu **build.gradle hatası DEĞİL** — dosya kilidi. Gördüğün kırmızılar (`rootProject.ext.compileSdkVersion`, `$androidxAppCompatVersion` vb.) de gerçek hata değil; Gradle sync bu kilit yüzünden başarısız olunca AS değişkenleri çözemediği için kırmızı gösterir. Sync düzelince kaybolurlar. Üç katmanlı kök neden + çözüm:

1. **Aynı anda iki Gradle daemon.** CLI derlemesi + açık Android Studio aynı `build` klasörüne dokununca biri diğerinin dosyasını silemez → `mergeDebugJniLibFolders FAILED` gibi. **Kural: AS'de derlerken CLI'dan aynı anda derleme.** Takılırsa: tüm `java` süreçlerini durdur (`Get-Process java | Stop-Process -Force`), `build` klasörlerini sil, tek araçtan derle.

2. **Gradle'ın dosya sistemi izlemesi (VFS watch)** build dosyalarına tutamaç açık tutar. `android/gradle.properties`'e ekle:
   ```
   org.gradle.vfs.watch=false
   ```

3. **Asıl kalıcı sebep — Windows Defender / boşluklu yol.** Gerçek zamanlı tarama `build`'e yazılan her `.so`/dex/report dosyasını anlık kilitler; hata her seferinde farklı dosyada çıkmasının sebebi budur. İki kesin çözüm (Patika'da **B işe yaradı**):
   - **A) Defender istisnası:** Virüs ve tehdit koruması → Dışlamalar → proje klasörü + `%USERPROFILE%\.gradle` ekle. (Ya da yönetici PowerShell: `Add-MpPreference -ExclusionPath "<proje>","$env:USERPROFILE\.gradle"`.)
   - **B) Projeyi boşluksuz kısa yola taşı** (ör. `E:\...\claude programlar\proje-app` → `C:\proje-app`). Capacitor yolları göreli olduğu için taşıma güvenli; taşıdıktan sonra `android/local.properties`'teki `sdk.dir`'i doğrula, eski konumdan gelen bayat `build` klasörlerini sil. Patika'da boşluklu `E:\ksaim\claude programlar\` yolu `C:\patika-app`'e taşınınca `clean` + `assembleDebug` ilk denemede geçti.

**Belirti-çözüm özeti:** Hata farklı dosyalarda dönüyorsa → ortam (Defender/yol). Hep aynı `merged_jni_libs`'te ve iki araç çalışıyorsa → daemon çakışması.

---

## 11. Test döngüsü (Play Store'a koymadan)
...
## 12. Her yeni projede tekrarlanacak kontrol listesi
...
- [ ] Konum/kamera gibi native izin gerektiren özellik varsa AndroidManifest.xml'e ilgili `<uses-permission>` satırları eklendi mi? (bkz. §9)
